...
library Encoded {
    ...
    uint256 internal constant MASK_ADDRESS = 0x00ffffffffffffffffffffffffffffffffffffffff; // add 2 zeros to silence warning
    ...
    /**
     * @notice Internal function to set an address in an encoded bytes32 using an offset
     * @dev This function can overflow
     * @param encoded The previous encoded value
     * @param value The value to encode
     * @param offset The offset
     * @return newEncoded The new encoded value
     */
    function setAddress(bytes32 encoded, address value, uint256 offset) internal pure returns (bytes32 newEncoded) {
        return set(encoded, uint256(uint160(value)), MASK_ADDRESS, offset);
    }
    ...
    /**
     * @notice Internal function to decode a bytes32 sample into an address using an offset
     * @dev This function can overflow
     * @param encoded The encoded value
     * @param offset The offset
     * @return value The decoded address
     */
    function decodeAddress(bytes32 encoded, uint256 offset) internal pure returns (address value) {
        assembly {
            value := and(shr(offset, encoded), MASK_ADDRESS)
        }
    }
}
